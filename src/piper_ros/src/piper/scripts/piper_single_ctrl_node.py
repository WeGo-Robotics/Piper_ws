...
class C_PiperRosNode(Node):
    def __init__(self) -> None:
        ...
        qos_profile = QoSProfile(
            history=QoSHistoryPolicy.KEEP_LAST,                  # 마지막 메시지만 유지, 큐에서 최대 메시지 수만 유지
            depth=10,                                            # 큐의 깊이: 메시지 버퍼 크기 (10개로 설정)
            reliability=ReliabilityPolicy.RELIABLE,              # 신뢰성 있는 메시지 전송
            durability=DurabilityPolicy.VOLATILE,                 # 메시지를 수신 후 저장하지 않음 (실시간성 우선)
        )
        ...
        self.create_subscription(JointState, 'joint_ctrl_single', self.joint_callback, qos_profile)
        self.create_subscription(Bool, 'enable_flag', self.enable_callback, 10)
        self.create_subscription(Bool, 'init_pos', self.init_callback, 10)
        self.create_subscription(Bool, 'gripper_ctrl', self.gripper_callback, 10)

        # 초기화 시 한 번만 /init_pos 발행
        self.init_pos_msg = Bool()
        self.init_pos_msg.data = True  # 또는 원하는 초기값 설정
        # self.init_pos_pub.publish(self.init_pos_msg)
    ...
    def joint_callback(self, joint_data):
        ...
        joint_1 = round(joint_data.position[0]*factor)
        joint_2 = round(joint_data.position[1]*factor)
        joint_3 = round(joint_data.position[2]*factor)
        joint_4 = round(joint_data.position[3]*factor)
        joint_5 = round(joint_data.position[4]*factor)
        joint_6 = round(joint_data.position[5]*factor)
        ...
        if(self.GetEnableFlag()):
            ...
            if(self.gripper_exist):
                ...
                else: self.piper.GripperCtrl(abs(gripper), 1000, 0x01, 0)
